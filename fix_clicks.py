...
async def register_clicks(
    ...
    await request.app.state.arq_pool.enqueue_job('process_clicks_job', batch.dict(), user_id)
...
async def process_click_batch(batch: ClickBatch, user_id: str, db: AsyncSession):
    ...
    dedup_key = f"dedup:{user_id}:{batch.timestamp}"
    is_new = await redis_client.set(dedup_key, "1", ex=20, nx=True)
    if not is_new:
        import asyncio
    ...
    if hasattr(batch, 'used_autoclicker_seconds') and batch.used_autoclicker_seconds > 0:
        ...
        actual_used = min(batch.used_autoclicker_seconds, current_sec)
        if actual_used > 0:
            inventory["autoclicker_seconds"] -= actual_used
            auto_clicks_pool += actual_used * 2
            
    if hasattr(batch, 'used_touchme_seconds') and batch.used_touchme_seconds > 0:
        current_touch = inventory.get("touchme_seconds", 0)
        actual_used_touch = min(batch.used_touchme_seconds, current_touch)
        if actual_used_touch > 0:
            inventory["touchme_seconds"] -= actual_used_touch
            auto_clicks_pool += actual_used_touch * 20
    ...
    clan_tax = 0.0
    if clan_id:
        clan_tax = new_coins_earned * 0.10
        user_keep = new_coins_earned - clan_tax
    ...
    pipe.sadd("pending_db_sync", user_id)
    pipe.incrby("global_clicks_buffer", total_added)
    ...
    if clan_id and clan_tax > 0:
        ...
        if current_clan_coins:
            msg = json.dumps({"type": "clan_update", "clan_id": clan_id, "egg_coins": float(current_clan_coins), "tax": clan_tax})
...
with open('j:\\PROYECTOS\\Egg\\backend\\app\\api\\v1\\clicks.py', 'w', encoding='utf-8') as f:
    f.write(code)
